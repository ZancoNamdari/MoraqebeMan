...
from tests.factories.jwt_factory import make_token
...
@override_settings(JWT_SIGNING_KEY="testkey")
class FamilyProfileTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.token = make_token(user_id=1, role="family")
    ...
    def test_non_family_role_rejected(self):
        token = make_token(user_id=2, role="caregiver")
...
class PatientTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.token = make_token(user_id=10, role="family")
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.token}")
    ...
    def test_different_family_cannot_see_this_patient(self):
        ...
        other_token = make_token(user_id=999, role="family")
...
class QuestionnaireTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        token = make_token(user_id=20, role="family")
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        create = self.client.post("/api/patients/", VALID_PATIENT, format="json")
