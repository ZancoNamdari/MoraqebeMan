...
import {
  Users, HeartHandshake, UserCog, AlertTriangle,
...
function StatCard({ icon: Icon, count, label, pendingCount, colorClass, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-start gap-2 rounded-xl border border-slate-200 bg-white p-4 text-right shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
      <span className={`flex h-10 w-10 items-center justify-center rounded-lg ${colorClass}`}>
        <Icon className="h-5 w-5" />
      </span>
      <p className="text-2xl font-bold text-slate-900">{toPersianDigits(count)}</p>
...
function ActionCard({ icon: Icon, title, description, badge, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      className="relative flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 text-right shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
      ...
      {badge !== undefined && badge > 0 && (
        <span className="absolute -top-2 -left-2 flex h-6 min-w-6 items-center justify-center rounded-full bg-red-600 px-1.5 text-[11px] font-bold text-white">
...
export default function DashboardPage() {
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-5">
        <h1 className="text-lg font-bold text-slate-900">{profile?.company_name || "پنل آژانس"}</h1>
        <p className="text-xs text-slate-500">نمای کلی وضعیت آژانس</p>
      </div>
        ...
        <div className="space-y-5">
          ...
          <Card className="border-slate-200">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-base text-slate-900">اطلاعات آژانس</CardTitle>
              {!editing && (
                <Button variant="outline" size="sm" className="gap-1.5 border-slate-300 text-blue-700 hover:bg-slate-50" onClick={() => setEditing(true)}>
            ...
            <CardContent className="space-y-3">
              {editing ? (
                ...
                <>
                  <p className="text-sm"><span className="text-slate-500">نام شرکت: </span>{profile.company_name || "—"}</p>
                  <p className="text-sm"><span className="text-slate-500">شماره مجوز: </span>{profile.license_number ? toPersianDigits(profile.license_number) : "—"}</p>
                </>
              )}

              <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3">
                <div>
                  <p className="text-xs text-slate-500">کد عضویت آژانس</p>
                  <p dir="ltr" className="text-left text-lg font-bold tracking-wider text-blue-700">{profile.access_code}</p>
                </div>
                <Button size="sm" variant="outline" className="gap-1.5 border-slate-300 bg-white text-blue-700 hover:bg-slate-50" onClick={handleCopyCode}>
          ...
          <div>
            <h2 className="mb-2 px-1 text-sm font-semibold text-slate-900">افراد</h2>
          ...
          <div>
            <h2 className="mb-2 px-1 text-sm font-semibold text-slate-900">رسیدگی و بررسی</h2>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <ActionCard
