...
export default function MatchWeightsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-secondary/50 via-background to-background pb-10">
      <AppHeader title="تنظیم وزن معیارهای تطابق" maxWidth="max-w-2xl">
          ...
          <>
            <Card className="border-border bg-secondary/30">
              <CardContent className="pt-4 text-xs text-muted-foreground">
                ...
                {isDefault && <p className="mt-2 font-medium text-foreground">در حال حاضر از تنظیمات پیش‌فرض استفاده می‌شود.</p>}
              </CardContent>
            </Card>
            ...
            <Card className="border-border">
              <CardHeader><CardTitle className="text-sm text-foreground">مقایسه دو به دوی معیارها</CardTitle></CardHeader>
                    ...
                    return (
                      <div key={key} className="space-y-1.5 rounded-lg border border-border p-3">
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-medium text-foreground">{MCDM_CRITERIA_LABELS[a]}</span>
                          <span className="text-muted-foreground">در برابر</span>
                          <span className="font-medium text-foreground">{MCDM_CRITERIA_LABELS[b]}</span>
                        </div>
            ...
            {computedWeights && (
              <Card className="border-border bg-secondary/30">
                <CardHeader><CardTitle className="text-sm text-foreground">وزن‌های محاسبه‌شده</CardTitle></CardHeader>
                <CardContent className="space-y-1 text-xs">
                  {Object.entries(computedWeights).map(([criterion, weight]) => (
                    <div key={criterion} className="flex items-center justify-between">
                      <span>{MCDM_CRITERIA_LABELS[criterion] || criterion}</span>
                      <span className="font-medium text-primary-strong">{(weight * 100).toFixed(1)}٪</span>
                    </div>
                  ))}
                  {consistencyRatio !== null && (
                    <p className={cn("mt-2 text-xs", consistencyRatio < 0.10 ? "text-emerald-700" : "text-primary-strong")}>
